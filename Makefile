...
ifneq ($(DARWIN),)
  $(info >>> WARNING - UnifiedMemoryStreams is not supported on Mac OSX - waiving sample <<<)
  SAMPLE_ENABLED := 0
endif
...
radixCuda.o: radix.cu
	$(NVCC) $(INCLUDES) $(ALL_CCFLAGS) $(GENCODE_FLAGS) -o $@ -c $<

caProj: radixCuda.o radix.o
...
clean:
	rm -f caProj radixCuda.o radix.o
	rm -rf ../../bin/$(OS_ARCH)/$(OSLOWER)/$(TARGET)$(if $(abi),/$(abi))/caProj
